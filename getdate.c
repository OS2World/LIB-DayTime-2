...
static char RCS[] =
	"$Id: getdate.y,v 1.5 1998/10/18 17:33:43 rommel Exp rommel $";
#endif	/* !defined(lint) && !defined(SABER) */


#define EPOCH		1970
#define HOUR(x)		((xtime_t)(x) * 60)
...
static xtime_t	yyDayOrdinal;
static xtime_t	yyDayNumber;
static int	yyHaveDate;
static int	yyHaveDay;
static int	yyHaveRel;
static int	yyHaveTime;
static int	yyHaveZone;
static xtime_t	yyTimezone;
static xtime_t	yyDay;
static xtime_t	yyHour;
static xtime_t	yyMinutes;
static xtime_t	yyMonth;
static xtime_t	yySeconds;
static xtime_t	yyYear;
static MERIDIAN	yyMeridian;
static xtime_t	yyRelMonth;
static xtime_t	yyRelSeconds;
...
static const short yyrline[] = { 0,
   229,   230,   233,   236,   239,   242,   245,   248,   251,   257,
   263,   270,   276,   286,   290,   294,   301,   305,   309,   315,
   319,   324,   330,   334,   339,   343,   350,   354,   357,   360,
   363,   366,   369,   372,   375,   378,   381,   386,   414,   417
};
#endif
    ...
    DSTMODE	DSTmode)
{
    ...
    DaysInMonth[1] = Year % 4 == 0 && (Year % 100 != 0 || Year % 400 == 0)
		    ? 29 : 28;
    if (Year < EPOCH || Year > 2099 /* was: 1999 ! */
     || Month < 1 || Month > 12
	...
	return -1;
    Julian += tod;
    xJulian = (time_t) Julian;
    if (DSTmode == DSTon
     || (DSTmode == DSTmaybe && localtime(&xJulian)->tm_isdst))
	Julian -= 60 * 60;
    return Julian;
}
    ...
    xtime_t	Future)
{
    xtime_t	StartDay;
    xtime_t	FutureDay;
    time_t	xStart = (time_t) Start, xFuture = (time_t) Future;

    StartDay = (localtime(&xStart)->tm_hour + 1) % 24;
    FutureDay = (localtime(&xFuture)->tm_hour + 1) % 24;
    ...
    xtime_t	DayNumber)
{
    ...
    now = (time_t) Start;
    tm = localtime(&now);
    xnow = Start + SECSPERDAY * ((DayNumber - tm->tm_wday + 7) % 7);
    xnow += 7 * SECSPERDAY * (DayOrdinal <= 0 ? DayOrdinal : DayOrdinal - 1);
    return DSTcorrect(Start, xnow);
}
    ...
    struct timeb	*now)
{
	...
	ftz.timezone = (long) difftm (&gmt, localtime (&ftz.time)) / 60;

	if (localtime(&ftz.time)->tm_isdst)
	  ftz.timezone += 60;
    }
